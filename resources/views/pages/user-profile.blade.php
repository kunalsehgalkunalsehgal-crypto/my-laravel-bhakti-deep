...
/* Compact Review Pandit flow on profile bookings */
.profile-review-collapse {
    margin-top: 12px;
}
...
.profile-review-shell .review-panel-heading {
    margin-bottom: 12px;
}

.profile-review-shell .review-panel-heading h3 {
    font-size: 18px;
    line-height: 1.25;
}

.profile-review-shell .review-panel-heading p {
    margin-top: 5px;
    font-size: 11px;
}

.profile-review-shell .review-flow-steps {
    margin-bottom: 12px;
}

.profile-review-shell .review-verification-gate {
    padding: 13px;
    border-radius: 12px;
}
                        ...
                        <div class="profile-card">
                            ...
                            @forelse ($bookings as $booking)
                                @php
                                    ...
                                    $bookingTypeKey = strtolower($type);
                                    $existingUserReview = $type !== 'Diya'
                                        ? $booking->reviews?->firstWhere('review_by', 'user')
                                        : null;

                                    $reviewTarget = $bookingTypeKey.':'.$booking->id;
                                    $reviewCollapseId = 'profile-review-'.$bookingTypeKey.'-'.$booking->id;

                                    /*
                                     * Keep review collapsed on a normal page load.
                                     * Re-open only after this exact booking's OTP/review POST,
                                     * so the user does not need to click Review Pandit again.
                                     */
                                    $reviewShouldOpen = $type !== 'Diya'
                                        && $booking->status === 'completed'
                                        && (
                                            session('review_feedback_target') === $reviewTarget
                                            || (
                                                old('booking_type') === $bookingTypeKey
                                                && (string) old('booking_id') === (string) $booking->id
                                            )
                                        );
                                @endphp
                                ...
                                <div class="booking-item">
                                    ...
                                    <div class="booking-actions">
                                        ...
                                        @else
                                            ...
                                            @if($booking->status === 'completed')
                                                <button
                                                    class="btn {{ $existingUserReview ? 'btn-outline-saffron' : 'btn-saffron' }} btn-sm rounded-pill profile-review-toggle"
                                                    type="button"
                                                    data-bs-toggle="collapse"
                                                    data-bs-target="#{{ $reviewCollapseId }}"
                                                    aria-expanded="{{ $reviewShouldOpen ? 'true' : 'false' }}"
                                                    aria-controls="{{ $reviewCollapseId }}"
                                                >
                                                    <i class="bi {{ $existingUserReview ? 'bi-star-fill' : 'bi-star' }}"></i>
                                                    {{ $existingUserReview ? 'View Review' : 'Review Pandit' }}
                                                </button>
                                            @endif
                                        @endif
                                    ...
                                    @if($type !== 'Diya' && $booking->status === 'completed')
                                        <div
                                            class="collapse profile-review-collapse {{ $reviewShouldOpen ? 'show' : '' }}"
                                            id="{{ $reviewCollapseId }}"
                                            data-profile-review-target="{{ $reviewTarget }}"
                                        >
...
<script>
    ...
    @if(session('review_feedback_target'))
        const reviewFeedbackPanel = document.querySelector(
            '[data-profile-review-target="{{ session('review_feedback_target') }}"]'
        );

        if (reviewFeedbackPanel) {
            window.setTimeout(function () {
                reviewFeedbackPanel.scrollIntoView({
                    behavior: 'smooth',
                    block: 'center'
                });
            }, 150);
